// src/__tests__/ClientTransactions.test.tsx
...
describe("ClientTransactions - Filtering Feature", () => {
    ...
    test("renders all transactions initially", async () => {
        ...
        await waitFor(() => {
            const elements = within(tableBody).queryAllByText((content) => content.includes(transactions[0].Payee.trim()));
            expect(elements[0]).toBeInTheDocument();
        });

        // Ensure all transactions are visible
        transactions.slice(0, 20).forEach(transaction => {
            const elements = within(tableBody).queryAllByText((content) => content.includes(transaction.Payee.trim()));
            expect(elements[0]).toBeInTheDocument();
        });
    });
    ...
    test("filters transactions based on Payee input", async () => {
        ...
        await waitFor(() => {
            const elements = within(tableBody).getAllByText((content) => content.includes(SEARCH_PAYEE));
            expect(elements[0]).toBeInTheDocument();
        });
    ...
    test("filters transactions based on amount threshold", async () => {
        ...
        rows.forEach(row => {
            ...
            if (!amountCell) {
                throw new Error("Amount cell not found");
            }
    ...
    test("combines payee and amount threshold filters", async () => {
        ...
        rows.forEach(row => {
            ...
            if (!payeeCell || !amountCell) {
                throw new Error("Payee or amount cell not found");
            }
